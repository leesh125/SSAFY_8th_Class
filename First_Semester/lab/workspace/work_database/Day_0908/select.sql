...
select employee_id, concat(first_name, ' ', last_name) as name, salary, salary * 12 as '연봉'
from employees;


-- 모든 사원의 사번, 이름, 급여, 급여 * 12 (연봉), 커미션, 커미션포함 연봉 검색.
select employee_id, concat(first_name, ' ', last_name) as name, salary, salary * 12 as '연봉',
		commission_pct as "커미션", (salary+salary*ifnull(commission_pct,0)) * 12 as '커미션 포함 연봉'
from employees;
