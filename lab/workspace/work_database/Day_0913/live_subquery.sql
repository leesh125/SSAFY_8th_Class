...
from employees e join(  select distinct department_id
						from employees
						where salary < (select avg(salary) 
										from employees)
					) a
...
select
	(select sum(salary) from employees where department_id = 50) as sum50,
    (select avg(salary) from employees where department_id = 60) as sum50,
    (select max(salary) from employees where department_id = 90) as sum50,
    (select min(salary) from employees where department_id = 90) as sum50;
